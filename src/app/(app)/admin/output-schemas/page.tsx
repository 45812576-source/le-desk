...
const STATUS_COLOR: Record<string, "green" | "yellow" | "cyan" | "red"> = {
  draft: "cyan",
...
export default function AdminOutputSchemasPage() {
  const { user } = useAuth();
  ...
  const fetchAll = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([s, sk]) => { setSchemas(s); setSkills(sk); })
      .catch(() => {})
  ...
  async function generate(skillId: number) {
    ...
    try {
      await apiFetch(`/admin/output-schemas/generate?skill_id=${skillId}`, { method: "POST" });
      fetchAll();
    } catch (e: unknown) {
      alert(e instanceof Error ? e.message : "推导失败");
    } finally {
      setGenerating(null);
    }
  }
  ...
  async function approve(schemaId: number) {
    if (!confirm("确认审批通过此 Schema？通过后不可再编辑。")) return;
    try {
      await apiFetch(`/admin/output-schemas/${schemaId}/approve`, { method: "POST" });
      fetchAll();
    } catch (e: unknown) {
  ...
  function startEdit(schema: OutputSchema) {
    ...
    setEditJson(JSON.stringify(schema.schema_json, null, 2));
    setEditError("");
  }
  ...
  async function saveEdit(schemaId: number) {
    ...
    try {
      await apiFetch(`/admin/output-schemas/${schemaId}`, {
        method: "PUT",
        body: JSON.stringify({ schema_json: parsed }),
      });
      setEditingId(null);
      fetchAll();
    } catch (e: unknown) {
  ...
  // Group schemas by skill for cleaner view
  const bySkill: Record<number, OutputSchema[]> = {};
  schemas.forEach((s) => {
    if (!bySkill[s.skill_id]) bySkill[s.skill_id] = [];
    bySkill[s.skill_id].push(s);
  });

  return (
    <PageShell title="输出Schema" icon={ICONS.outputSchema}>
      {/* Filters */}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          <div className="text-[10px] text-gray-500 text-center">
            选择 Skill 后点击「LLM推导」自动生成 Schema
          </div>
          {skills.length > 0 && (
              ...
              <React.Fragment key={s.id}>
                <tr key={s.id} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">
                    <PixelBadge color={STATUS_COLOR[s.status] || "cyan"}>
                  ...
                  <td className="px-3 py-2 flex gap-1">
                    ...
                    {isSuperAdmin && s.status === "pending_review" && (
                      <button
                        ...
                        className="text-[10px] font-bold text-green-600 hover:underline ml-2"
                      >
                        审批通过
                      </button>
                    )}
                    <button
                      onClick={() => generate(s.skill_id)}
                      disabled={generating === s.skill_id}
                      className="text-[10px] font-bold text-[#805AD5] hover:underline ml-2 disabled:opacity-40"
                    >
                ...
                {expandedId === s.id && (
                  <tr key={`${s.id}-view`}>
                    <td colSpan={5} className="px-4 py-3 bg-gray-50 border-b border-gray-200">
                      <pre className="text-[10px] font-mono overflow-auto max-h-48">
                ...
                {editingId === s.id && (
                  <tr key={`${s.id}-edit`}>
                    <td colSpan={5} className="px-4 py-3 bg-[#FEFCBF] border-b-2 border-[#B7791F]">
                      <textarea
                        value={editJson}
                      ...
                      />
                      {editError && (
                        <div className="mt-1 text-xs text-red-600 font-bold">{editError}</div>
                      )}
      ...
      {/* Trigger generate for specific skill */}
      {schemas.length > 0 && (
          ...
          <div className="flex gap-2 flex-wrap">
            {skills.filter((sk) => !schemas.some((s) => s.skill_id === sk.id)).map((sk) => (
              <PixelButton
                ...
                size="sm"
                variant="secondary"
                disabled={generating === sk.id}
                onClick={() => generate(sk.id)}
              >
