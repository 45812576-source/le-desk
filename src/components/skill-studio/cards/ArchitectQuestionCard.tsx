...
export const ArchitectQuestionCard = memo(function ArchitectQuestionCard({
  ...
  if (answered) {
    return (
      <div className={`mx-3 my-1 px-2.5 py-1.5 border ${theme.border} bg-white/60 text-[9px] font-mono opacity-60`}>
        <div className="flex items-center gap-2">
          <span className={`text-[7px] font-bold uppercase ${theme.text}`}>{fwLabel}</span>
          <span className="text-gray-500 flex-1 truncate">{question.question}</span>
          {answeredText && (
  ...
  return (
    <div className={`mx-3 my-2 border-2 ${theme.border} bg-white text-[9px] font-mono`}>
      ...
      <div className="px-2.5 py-2">
        <p className="font-bold text-[#1A202C] text-[10px] mb-2">{question.question}</p>
        ...
        {hasOptions ? (
          <div className="space-y-1 mb-2">
            {question.options!.map((opt, i) => (
              <button
                ...
                className={`w-full text-left px-2.5 py-1.5 border ${theme.border} hover:${theme.bg} hover:${theme.text} transition-colors text-gray-600 flex items-center gap-2`}
              >
            ...
            {!showCustom ? (
              <button
                ...
                className="w-full text-left px-2.5 py-1.5 border border-gray-200 text-gray-400 hover:border-gray-400 hover:text-gray-600 transition-colors"
              >
              ...
              <div className="flex gap-1">
                <input
                  ...
                  placeholder="输入你的回答..."
                  autoFocus
                  className={`flex-1 px-2 py-1 border ${theme.border} text-[9px] focus:outline-none focus:border-[#00D1FF]`}
                />
                <button
          ...
          <div className="flex gap-1 mb-2">
            <input
              ...
              placeholder="输入你的回答..."
              autoFocus
              className={`flex-1 px-2 py-1.5 border ${theme.border} text-[9px] focus:outline-none focus:border-[#00D1FF]`}
            />
            <button
        ...
        {question.why && showWhy && (
          <p className="mt-1 text-[8px] text-gray-400 pl-3 border-l-2 border-gray-200">
            {question.why}
          </p>
        )}
      </div>
